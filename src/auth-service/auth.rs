...
impl Auth for AuthService {
    async fn sign_in(
        ...
        let result: Option<String> = self.users_service.lock().unwrap().get_user_uuid(req.username.clone(), req.password.clone()); // Get user's uuid from `users_service`. Panic if the lock is poisoned.
    ...
    async fn sign_up(
        ...
        let result: Result<(), String> = self.users_service.lock().unwrap().create_user(req.username.clone(), req.password.clone()); // Create a new user through `users_service`. Panic if the lock is poisoned.
    ...
    async fn sign_out(
        ...
        self.sessions_service.lock().unwrap().delete_session(&req.session_token); // Delete session using `sessions_service`. Panic if the lock is poisoned.
